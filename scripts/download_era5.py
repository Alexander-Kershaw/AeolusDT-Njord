...
def main() -> int:
    ...
    if args.height == 10:
        ...
        var_u = "100m_u_component_of_wind"
        var_v = "100m_v_component_of_wind"
        u_name, v_name = "u100", "v100"

    # explicit year/month/day lists (more robust time ranges)
    ...
    area = [r.lat_max, r.lon_min, r.lat_min, r.lon_max]  # [N, W, S, E] bounds
    out_dir = Path("data_lake/bronze/era5") / args.region
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"era5_single_levels_{u_name}{v_name}_{args.start}_to_{args.end}.nc"

    req = {
        "product_type": "reanalysis",
        "variable": [var_u, var_v],
        "year": years,
        "month": months,
        "day": days,
    ...
    print("Height:", args.height, "m")
    print("Variables:", var_u, ",", var_v)
